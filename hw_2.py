# -*- coding: utf-8 -*-
"""ДЗ2_Кравченко Сергій Сергійович.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1mcb7TpABB6N7tyOSYxcHmRTUaNPicEtN

# Завдання 1
Виріши аналітично задачу з текстовою умовою

Над проектом працювало 3 типи спеціалістів - дизайнери, розробники та тестувальники. Ми знаємо кількість годин на кожному етапі розробки та кількість грошей, які закладені на виконання кожного етапу в якості собівартості.
Всю розробку менеджер розділив на 3 етапи: створення дизайну, розробка програмної частини, фінальне тестування.

На першому етапі відпрацьовано 20 годин дизайнером та 3 години розробниками - на даному етапі до смети закладено 775 доларів.
На другому етапі дизайнери відпрацювали 8 годин, розробники - 28, а тестувальники - 4. Даний етап було оцінено в 1012 доларів.
На фінальному етапі дизайнери відпрацювали 4 години, розробники - 12, а тестувальники 32. Трудові ресурси на цьому етапі оцінені в 696 доларів.

Знайди вартість години у кожного спеціаліста шляхом вирішення системи лінійних рівнянь, базуючись на даних по етапах розробки.

Розв'язок:

Крок 1: Позначення змінних

x — вартість години роботи дизайнера (дол/год)

y — вартість години роботи розробника (дол/год)

z — вартість години роботи тестувальника (дол/год)


\begin{array}{cc}
1) \quad 20x + 3y + 0z = 775 \\
2) \quad 8x + 28y + 4z = 1012 \\
3) \quad 4x + 12y + 32z = 696
\end{array}\
$$
A =
\left (\begin{array}{cc}
20 & 3 & 0 \\
8 & 28 & 4 \\
4 & 12 & 32
\end{array}\right)
$$
\
$$
b =
\left (\begin{array}{cc}
775 \\
1012 \\
696
\end{array}\right)
$$

*Розв'язувати можна будь-яким методом, але метод Крамера, потенційно, буде найлегшим*




Визначник Δ
$$
\\Delta = \left|A\right| = 20 \cdot \begin{vmatrix} 28 & 4 \\ 12 & 32 \end{vmatrix} - 3 \cdot \begin{vmatrix} 8 & 4 \\ 4 & 32 \end{vmatrix} + 0 \cdot \begin{vmatrix} 8 & 28 \\ 4 & 12 \end{vmatrix}
$$
\
Обчислюємо:
$$
\Delta = 20(28 \cdot 32 - 4 \cdot 12) - 3(8 \cdot 32 - 4 \cdot 4) + 0(8 \cdot 12 - 28 \cdot 4)=20(848) - 3(240) + 0 = 16960 - 720 = 16240
\
$$


Тоді:
$$
\Delta = 16240
\
$$

**Обчислення визначників для змінних:**

Для x: Замінимо перший стовпець матриці коефіцієнтів на стовпець вільних членів:
$$
\Delta_x = \begin{vmatrix}
775 & 3 & 0 \\
1012 & 28 & 4 \\
696 & 12 & 32
\end{vmatrix}
$$

Обчислюємо:
$$
\Delta_x = 775(28 \cdot 32 - 4 \cdot 12) - 3(1012 \cdot 32 - 4 \cdot 696) + 0 = 657200 - 88800 = 568400
\
$$
\
Для y: Замінимо перший стовпець матриці коефіцієнтів на стовпець вільних членів:
$$
\Delta_y = \begin{vmatrix}
20 & 775 & 0 \\
8 & 1012 & 4 \\
4 & 696 & 32
\end{vmatrix}
$$

Обчислюємо:
$$
\Delta_y = 20 \cdot 29600 - 775 \cdot 240 = 592000 - 186000 = 406000
\
$$
\
Для z: Замінимо перший стовпець матриці коефіцієнтів на стовпець вільних членів:
$$
\Delta_z = \begin{vmatrix}
20 & 3 & 775 \\
8 & 28 & 1012 \\
4 & 12 & 696
\end{vmatrix}
$$

Обчислюємо:
$$
\Delta_z = 20 \cdot 7344 - 3 \cdot 1520 + 775 \cdot (-16) = 146880−4560−12400 = 129920
\
$$
\
Знаходимо x:
$$
x = \frac{\Delta_x}{\Delta} = \frac{568400}{16240} = 35
$$  
Знаходимо y:
$$
y = \frac{\Delta_y}{\Delta} = \frac{406000}{16240} = 25
$$
Знаходимо z:
$$
z = \frac{\Delta_z}{\Delta} = \frac{129920}{16240} = 8
$$

**Відповідь:**

- Вартість години дизайнера: **35 доларів/год**
- Вартість години розробника: **25 доларів/год**
- Вартість години тестувальника: **8 доларів/год**
"""



"""Перевірка: $x+y+z = 68$

# Завдання 2
- Задай матрицю та вектор вільних членів для наступної системи:

![image.png](data:image/png;base64,)

$$
A =
\left (\begin{array}{cc}
-1 & 1 & 2 \\
0 & -1 & -3 \\
4 & -3 & 2
\end{array}\right)
$$
\
$$
B =
\left (\begin{array}{cc}
1 \\
-4 \\
7
\end{array}\right)
$$
"""

import numpy as np
A = np.array([
    [-1, 1, 2],
    [0, -1, -3],
    [4, -3, 2]
])

B = np.array([1, -4, 7])

# Розв'язання системи лінійних рівнянь за допомогою методу solve
x = np.linalg.solve(A, B)

# Виводимо розв'язок
print(x)

"""
- Напиши функцію для вирішення системи матричним методом
- Напиши функцію для вирішення системи методом Крамера"""

# Напиши функцію для вирішення системи матричним методом
import numpy as np
def solve_inv_matrix(a, b, verbose=False):
    """
    Solves the system of linear equations using the inverse matrix method.
    Parameters:
        a (numpy.ndarray): The matrix of coefficients.
        b (numpy.ndarray): The vector of constants.
        verbose (bool): If True, prints the inverse matrix.
    Returns:
        numpy.ndarray: Solution vector.
    """
    # Calculate the inverse of matrix a
    a_inv = np.linalg.inv(a)

    # Display the inverse matrix if verbose is True
    if verbose:
        print("Обернена матриця A:")
        print(a_inv)

    # Calculate the solution vector
    x = a_inv.dot(b)

    return x

# Define matrix A and vector B based on the user's input
A = np.array([[-1, 1, 2], [0, -1, -3], [4, -3, 2]])
B = np.array([1, -4, 7])

# Solve the system and display the result
print(f"Вектор рішення:\n{solve_inv_matrix(A, B, verbose=True)}")

# Напиши функцію для вирішення системи методом Крамера
import numpy as np

# Function to solve the system using Cramer's Rule
def solve_cramers_method(a, b):
    # Calculate the determinant of the original matrix A
    det_a = np.linalg.det(a)
    if abs(det_a) < 1e-10:
        return "Matrix A is singular and Cramer's Rule cannot be applied."

    # Initialize an empty list to store solutions for each variable
    solutions = []

    # Iterate over each column in matrix A
    for i in range(a.shape[1]):
        # Create a copy of matrix A to modify it
        modified_matrix = a.copy()
        # Replace the i-th column with vector B
        modified_matrix[:, i] = b
        # Calculate the determinant of the modified matrix
        det_modified = np.linalg.det(modified_matrix)

        # Store the value of the variable
        solutions.append(det_modified / det_a)

        # Print the intermediate determinant values
        print(f"Визначник матриці A{i+1} (заміна стовпця {i+1} на вектор B): {det_modified}")

    # Convert the solution list to a numpy array
    solutions = np.array(solutions)

    return solutions

# Example matrices
A = np.array([[-1, 1, 2], [0, -1, -3], [4, -3, 2]])
B = np.array([1, -4, 7])

# Calculate and print the determinant of A
det_A = np.linalg.det(A)
print(f"Визначник матриці A: {det_A}")

# Print the solution vector
solution = solve_cramers_method(A, B)
print(f"Вектор рішення: \n{solution}")